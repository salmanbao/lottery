...
contract Lottery is Ownable, Initializable, Testable {
    ...
    // State variables 
    // Instance of Safemoon token (collateral currency for lotto)
    IERC20 internal safemoon_;
    // Storing of the NFT
    ...
    // Project Reward Donation Pool
    address payable private prdpWallet_ = 0x970d98e0dCb7dDF4D4Fe6f43Fdb6541E6c19B428;
    // Burn wallet
    address payable private burnWallet_ = 0x970d98e0dCb7dDF4D4Fe6f43Fdb6541E6c19B428;
    ...
    struct LottoInfo {
        ...
        uint256 prizePoolInSafemoon;    // The amount of safemoon for prize money
        uint256 costPerTicket;      // Cost per ticket in $safemoon
     ...
     * @param   _prizePoolInSafemoon The amount of Safemoon available to win in this 
     *          lottery.
        ...
        returns(uint256 lotteryId)
    {
        ...
        // Ensuring that prize distribution total is 70%
        require(
            prizeDistributionTotal == 70,
            "Prize distribution is not 70%"
        );
        require(
            _prizePoolInSafemoon != 0 && _costPerTicket != 0,
        ...
        // split funds
        uint256 balance = IERC20(safemoon_).balanceOf(address(this));
        IERC20(safemoon_).transfer(burnWallet_,balance.mul(25).div(100));
        IERC20(safemoon_).transfer(prdpWallet_,balance.mul(5).div(100));
    ...
    function withdrawSafemoon(uint256 _amount) external onlyOwner() {
        safemoon_.transfer(
            msg.sender, 
            _amount
        );
        ...
        notContract()
    {
        ...
        require(
            _numberOfTickets <= 100,
            "Batch mint too large"
        );
        // Temporary storage for the check of the chosen numbers array
        uint256 numberCheck = uint256(sizeOfLottery_).mul(uint256(_numberOfTickets));
        ...
        // Transfers the required safemoon to this contract
        safemoon_.transferFrom(
            msg.sender, 
            address(this), 
            costWithDiscount
        );
    ...
    function claimReward(uint256 _lotteryId, uint256 _tokenId) external notContract() {
        ...
        allLotteries_[_lotteryId].prizePoolInSafemoon = allLotteries_[_lotteryId].prizePoolInSafemoon.sub(prizeAmount);
        // Transfering the user their winnings
        safemoon_.safeTransfer(address(msg.sender), prizeAmount);
    }
        ...
        notContract()
    {
        ...
        safemoon_.safeTransfer(address(msg.sender), totalPrize);
    }
     ...
     * @return  uint256: The prize amount in safemoon the user is entitled to 
     */
        ...
        returns(uint256) 
    {
        ...
        prize = allLotteries_[_lotteryId].prizePoolInSafemoon.mul(perOfPool);
