...
const { lotto, lottoNFT, generateLottoNumbers } = require('./settings.js')
...
describe('Lottery contract', function () {
  ...
  beforeEach(async () => {
    ...
    cakeInstance = await mock_erc20Contract.deploy(lotto.buy.cake)
    ...
    await cakeInstance.mint(lotteryInstance.address, lotto.newLotto.prize)
  ...
  describe('Creating a new lottery tests', function () {
    ...
    it('Nominal case', async function () {
      ...
      await expect(
        ...
        .withArgs(1, 0)
      //   console.log(
      //     await lotteryInstance.callStatic.getBasicLottoInfo(
  ...
  describe('Buying tickets tests', function () {
    ...
    it('Cost per ticket', async function () {
      let totalPrice = await lotteryInstance.costToBuyTickets(1, 10)
      ...
      assert.equal(
        oneCost.toString(),
        BigNumber(lotto.newLotto.cost.toString()),
        'Incorrect cost for batch buy of 10',
      )
    })
    ...
    it('Batch buying 1 tickets', async function () {
      ...
      let price = await lotteryInstance.costToBuyTickets(1, 1)
      ...
      await cakeInstance.connect(owner).approve(lotteryInstance.address, price)
      // Batch buying tokens
      await lotteryInstance.batchBuyLottoTicket(1, 1, ticketNumbers)
      // Testing results
      assert.equal(
        price.toString(),
        lotto.buy.one.cost,
    ...
    it('Batch buying 10 tickets', async function () {
      // Getting the price to buy
      let price = await lotteryInstance.costToBuyTickets(1, 10)
      // Generating chosen numbers for buy
      let ticketNumbers = generateLottoNumbers({
        numberOfTickets: 10,
        lottoSize: lotto.setup.sizeOfLottery,
      ...
      await lotteryInstance
        .connect(owner)
        .batchBuyLottoTicket(1, 10, ticketNumbers)
      // Testing results
    ...
    it('Batch buying 50 tickets', async function () {
      // Getting the price to buy
      let price = await lotteryInstance.costToBuyTickets(1, 50)
      // Generating chosen numbers for buy
      let ticketNumbers = generateLottoNumbers({
        numberOfTickets: 50,
        lottoSize: lotto.setup.sizeOfLottery,
      ...
      await lotteryInstance
        .connect(owner)
        .batchBuyLottoTicket(1, 50, ticketNumbers)
      // Testing results
      assert.equal(
        price.toString(),
  ...
  describe('Drawing numbers tests', function () {
    ...
    it('Setting winning numbers', async function () {
      ...
      // Drawing the numbers
      let tx = await (
        await lotteryInstance.connect(owner).drawWinningNumbers(1, 123456)
      ).wait()
    ...
    it('Invalid winning numbers (owner)', async function () {
      ...
      // Drawing the numbers
      await expect(
        lotteryInstance.connect(buyer).drawWinningNumbers(1, 1234),
      ).to.be.revertedWith(lotto.errors.invalid_admin)
    })
    ...
    it('Invalid winning numbers (already chosen)', async function () {
      ...
      await lotteryInstance.setCurrentTime(futureTime.toString())
      // Drawing the numbers
      await lotteryInstance.connect(owner).drawWinningNumbers(1, 1234)
      // Drawing the numbers again
      await expect(
        lotteryInstance.connect(owner).drawWinningNumbers(1, 1234),
  ...
  describe('Claiming tickets tests', function () {
    beforeEach(async () => {
      ...
      // Buying tickets
      // Getting the price to buy
      let prices = await lotteryInstance.costToBuyTicketsWithDiscount(1, 50)
      // Sending the buyer the needed amount of cake
      await cakeInstance.connect(buyer).mint(buyer.address, prices[2])
      // Approving lotto to spend cost
      await cakeInstance
        .connect(buyer)
        .approve(lotteryInstance.address, prices[2])
      // Generating chosen numbers for buy
      let ticketNumbers = generateLottoNumbers({
        numberOfTickets: 50,
        lottoSize: lotto.setup.sizeOfLottery,
    ...
    it('Claiming winning numbers (6 (all) match)', async function () {
      // Getting the price to buy
      let prices = await lotteryInstance.costToBuyTicketsWithDiscount(1, 1)
      // Sending the buyer the needed amount of cake
      await cakeInstance.connect(owner).transfer(buyer.address, prices[2])
      // Approving lotto to spend cost
      await cakeInstance
        .connect(buyer)
        .approve(lotteryInstance.address, prices[2])
      await lotteryInstance
        .connect(buyer)
        .batchBuyLottoTicket(1, 1, lotto.newLotto.win.winningNumbersArr)
      ...
      let userTicketIds = await lotteryNftInstance.getUserTickets(
        1,
        buyer.address,
      )
      // Drawing the numbers
      ...
      await lotteryInstance
        .connect(buyer)
        .claimReward(1, userTicketIds[50].toString())
    ...
    it('Claiming winning numbers (5 match)', async function () {
      ...
      altered[0] = 0
      await lotteryInstance.connect(buyer).batchBuyLottoTicket(1, 1, altered)
      // Setting current time so that drawing is correct
      // Getting the current block timestamp
      let currentTime = await lotteryInstance.getCurrentTime()
      // Converting to a BigNumber for manipulation
    ...
    it('Invalid claim (incorrect time)', async function () {
      ...
      await expect(
        lotteryInstance
          .connect(buyer)
          .claimReward(1, userTicketIds[25].toString()),
    ...
    it('Invalid claim (numbers out of range)', async function () {
      ...
      await lotteryInstance
        .connect(buyer)
        .batchBuyLottoTicket(1, 1, lotto.errorData.ticketNumbers)
      // Setting current time so that drawing is correct
      // Getting the current block timestamp
      let currentTime = await lotteryInstance.getCurrentTime()
      // Converting to a BigNumber for manipulation
    ...
    it('Invalid claim (ticket for different lottery)', async function () {
      ...
      // Getting the price to buy
      let prices = await lotteryInstance.costToBuyTicketsWithDiscount(2, 1)
      // Sending the buyer the needed amount of cake
      await cakeInstance.connect(owner).transfer(buyer.address, prices[2])
      // Approving lotto to spend cost
      await cakeInstance
      ...
      await lotteryInstance
        .connect(buyer)
        .batchBuyLottoTicket(2, 1, lotto.newLotto.win.winningNumbersArr)
      // Setting current time so that drawing is correct
      // Getting the current block timestamp
      currentTime = await lotteryInstance.getCurrentTime()
      // Converting to a BigNumber for manipulation
      ...
      await expect(
        lotteryInstance
          .connect(buyer)
          .claimReward(1, userTicketIds[0].toString()),
  ...
  describe('Batch claiming tickets tests', function () {
    beforeEach(async () => {
      ...
      // Buying tickets
      // Getting the price to buy
      let prices = await lotteryInstance.costToBuyTicketsWithDiscount(1, 49)
      // Sending the buyer the needed amount of cake
      await cakeInstance.connect(owner).transfer(buyer.address, prices[2])
      // Approving lotto to spend cost
      await cakeInstance
      ...
      await lotteryInstance
        .connect(buyer)
        .batchBuyLottoTicket(1, 49, ticketNumbers)
      // Getting the price to buy
      prices = await lotteryInstance.costToBuyTicketsWithDiscount(1, 1)
      // Sending the buyer the needed amount of cake
      await cakeInstance.connect(owner).transfer(buyer.address, prices[2])
      // Approving lotto to spend cost
      await cakeInstance
    ...
    it('Batch claiming winning numbers (multiple match)', async function () {
      ...
      await lotteryInstance.setCurrentTime(futureEndTime.toString())
      // Claiming winnings
      await lotteryInstance.connect(buyer).batchClaimRewards(1, userTicketIds)
      let buyerCakeBalanceAfter = await cakeInstance.balanceOf(buyer.address)
      // Tests
      assert.equal(
        buyerCakeBalanceBefore.toString(),
  ...
  describe('Upgrade functionality tests', function () {
    ...
    it('Invalid update size of lottery (non-owner)', async function () {
      ...
      await expect(
        lotteryInstance
          .connect(buyer)
          .updateSizeOfLottery(lotto.update.sizeOfLottery),
      ).to.be.revertedWith(lotto.errors.invalid_admin)
    })
    /**
    ...
    it('Update range of numbers', async function () {
      ...
      await lotteryInstance
        .connect(owner)
        .updateMaxRange(lotto.update.maxValidRange)
    ...
    it('Invalid update size of lottery (same as current)', async function () {
      ...
      await expect(
        lotteryInstance
          .connect(owner)
          .updateMaxRange(lotto.setup.maxValidRange),
      ).to.be.revertedWith(lotto.errors.invalid_size_update_duplicate)
    })
    /**
    ...
    it('Withdraw excess safemonn', async function () {
      let ownerCakeBalance = await cakeInstance.balanceOf(owner.address)
      let lotteryCakeBalance = await cakeInstance.balanceOf(
        lotteryInstance.address,
      )
      await lotteryInstance.connect(owner).withdrawSafemoon(lotteryCakeBalance)
      let ownerCakeBalanceAfter = await cakeInstance.balanceOf(owner.address)
      let lotteryCakeBalanceAfter = await cakeInstance.balanceOf(
        lotteryInstance.address,
      )
      // Tests
      assert.notEqual(
    ...
    it('Invalid withdraw excess cake (non owner)', async function () {
      ...
      await expect(
        lotteryInstance.connect(buyer).withdrawSafemoon(lotteryCakeBalance),
      ).to.be.revertedWith(lotto.errors.invalid_admin)
    })
  })
  ...
  //     // Getting the price to buy
  //     prices = await lotteryInstance.costToBuyTicketsWithDiscount(1, 50)
  //     // Sending the buyer the needed amount of cake
  //     await cakeInstance.connect(buyer).mint(buyer.address, prices[2])
  //     // Approving lotto to spend cost
  //     await cakeInstance
  ...
  //     let userInfo = await lotteryNftInstance.getUserTickets(1, buyer.address)
  //   })
  // })
})
